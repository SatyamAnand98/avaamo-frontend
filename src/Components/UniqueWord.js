...
export default function UniqueWord() {
    ...
    const handleUpload = () => {
        ...
        axios
            .post(
                "https://filesenseamd64.onrender.com/api/v1/files/get/unique-words",
                formData,
                {
                    headers: {
        ...
        <div>
            ...
            <div>
                {files.map((file, index) => (
                    <div
                        className="border w-64 ml-96 mt-2  flex justify-center"
                        key={index}
                    >
                        <input
                            type="file"
                ...
                {Array.isArray(value) &&
                    value.map((item) => (
                        <div className="border p-2 mt-4">
                            <h1 className="text-left font-bold ml-2 ">
                                {" "}
                                FileName: {item.fileName}
                            </h1>
                            <h1 className="text-left mt-4 ml-2 font-bold -mb-4">
                                WordCount:
                            </h1>
                            <br></br>
                            <div className="grid grid-cols-11 mb-4 gap-4">
                                {Object.entries(item.wordCount).map(
                                    ([word, count], index) => (
                                        <div className="col-span-1" key={word}>
                                            <div className="flex items-center">
                                                <h1 className="font-medium ml-2 text-xs">
                                                    {word}:
                                                </h1>
